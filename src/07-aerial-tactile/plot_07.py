"""
Assignment 07-Aerial-Tactile — Contact Inspection (Gazebo)
...
PLOT_DIR = os.path.join(SCRIPT_DIR, 'plots_07', 'gazebo')
LOG_DIR = os.path.expanduser('~/tk3lab-ws/logs/07-aerial-tactile')

RAD2DEG = 180.0 / math.pi
X_WALL = 2.0
L_BAR = 0.6
TIP_RADIUS = 0.02  # sphere collision radius (TacTip-scale, 20mm)
L_EFF = L_BAR + TIP_RADIUS  # tip surface contacts wall, not tip center
EE_Z_OFFSET = -0.125  # EE tip z-offset in body frame (from SDF)
K_WALL = 500.0
...
# EE tip surface position: pos + R(quat) @ [L_EFF, 0, EE_Z_OFFSET]
...
fig1.suptitle('07-Aerial-Tactile — Trajectory Tracking', fontsize=13)
...
fig2.suptitle('07-Aerial-Tactile — Contact & End-Effector', fontsize=13)

F_REQ = 2.0
ax = axes2[0]
ax.set_title('Contact Force [N]')
if f_contact is not None:
    ax.plot(t, -f_contact[:, 0], 'r', lw=0.8, label='spring |Fx|')
...
# Overlay Gazebo contact sensor force from npz if available
if isinstance(sim, np.lib.npyio.NpzFile) and 'contact_force' in sim and 'contact_flag' in sim:
    t_npz = sim['t']
    c_flag = sim['contact_flag']
    c_force = sim['contact_force']
    fx_gz = np.where(c_flag, np.abs(c_force[:, 0]), 0)
    ax.plot(t_npz, fx_gz, 'b', lw=0.6, alpha=0.7, label='gz sensor |Fx|')
...
ax.axhline(0.875, color='m', ls=':', lw=0.5, alpha=0.5, label='ridge z_bot=0.875')
ax.axhline(1.875, color='m', ls=':', lw=0.5, alpha=0.5, label='ridge z_top=1.875')
ax.set_xlabel('t [s]')
ax.set_ylabel('[m]')
ax.legend(fontsize=7)
ax.grid(True, alpha=0.3)
...
fig3.suptitle('07 Nominal vs Admittance-Filtered Trajectory', fontsize=13)
...
if has_nominal:
    ax.plot(t, p_nom[:, 0], 'r--', lw=0.6, alpha=0.7, label='setpoint (pre-AF)')
if has_desired:
    ax.plot(t, pos_d[:, 0], 'r:', lw=0.6, alpha=0.5, label='desired (post-AF)')
...
if has_nominal:
    ax.plot(t, p_nom[:, 1], 'g--', lw=0.6, alpha=0.7, label='setpoint (pre-AF)')
if has_desired:
    ax.plot(t, pos_d[:, 1], 'g:', lw=0.6, alpha=0.5, label='desired (post-AF)')
ax.set_xlabel('t [s]')
ax.legend(fontsize=7)
ax.grid(True, alpha=0.3)
...
if has_nominal:
    ax.plot(t, p_nom[:, 2], 'b--', lw=0.6, alpha=0.7, label='setpoint (pre-AF)')
if has_desired:
    ax.plot(t, pos_d[:, 2], 'b:', lw=0.6, alpha=0.5, label='desired (post-AF)')
ax.set_xlabel('t [s]')
ax.legend(fontsize=7)
ax.grid(True, alpha=0.3)
...
# Ridge frame rectangle (EE tip coordinates on wall)
ridge_y = [0, 1, 1, 0, 0]
ridge_z = [0.875, 0.875, 1.875, 1.875, 0.875]
ax.plot(ridge_y, ridge_z, 'm-', lw=2, alpha=0.5, label='ridge frame')
ax.plot(p_ee[:, 1], p_ee[:, 2], 'k', lw=0.8, label='EE tip path')
ax.plot(x[:, 1], x[:, 2], 'k--', lw=0.4, alpha=0.4, label='body path')
if has_nominal:
    ax.plot(p_nom[:, 1], p_nom[:, 2], 'b--', lw=0.6, alpha=0.7, label='setpoint (pre-AF)')
if has_desired:
    ax.plot(pos_d[:, 1], pos_d[:, 2], 'r:', lw=0.6, alpha=0.5, label='desired (post-AF)')
ax.set_xlabel('Y [m]')
ax.set_ylabel('Z [m]')
ax.set_aspect('equal')
ax.legend(fontsize=7)
...
fig4.suptitle('07 External Wrench Estimate (idealized r × F from EE penetration)', fontsize=13)
